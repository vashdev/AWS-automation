...
ARG_CNT=0
while [ $# -gt 1 -a $ARG_CNT -lt 15 ]; do
  case "$1" in
    --BATCH_FILE_TYPE=* | BATCH_FILE_TYPE=*)
      export BATCH_FILE_TYPE="${1#*=}"
      shift
      ;;
    --BATCH_FILE_S3_URL=* | BATCH_FILE_S3_URL=*)
      export BATCH_FILE_S3_URL="${1#*=}"
      shift
      ;;
  esac
  let ARG_CNT=$ARG_CNT+1
done
echo "BATCH_FILE_TYPE=$BATCH_FILE_TYPE; BATCH_FILE_S3_URL=$BATCH_FILE_S3_URL"
...
TMPLIB="${TMPDIR}/batch-lib-temp"
install -m 0600 /dev/null "${TMPFILE}" || error_exit "Failed to create temp file."

export EMR_MASTER_IP=`curl http://169.254.169.254/latest/meta-data/local-ipv4`
echo "EMR_MASTER_IP --------- $EMR_MASTER_IP"
...
fetch_and_run_zip () {
  ...
  if [ ! -z "${BATCH_LIB_S3_URL}" ]; then
	  #Create a temporary file and download the libraries zip file
	  echo "Downloading libraries .... ${TMPLIB}"
	  aws s3 cp "${BATCH_LIB_S3_URL}" - > "${TMPLIB}" || error_exit "Failed to download S3 libraries zip file from ${BATCH_LIB_S3_URL}"
	  # Create a temporary directory and unpack the zip file
	  unzip -q "${TMPLIB}" || error_exit "Failed to unpack library zip file."
  fi
  ...
  exec "${script}" "${@}" --context_param EMR_MASTER_IP=$EMR_MASTER_IP || error_exit " Failed to execute script."
}
